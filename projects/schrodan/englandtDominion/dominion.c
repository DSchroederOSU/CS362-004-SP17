...
int SeaHag(struct gameState *state) {
	int i;
	int currentPlayer = whoseTurn(state);
	for (i = 0; i < state->numPlayers; i++){
		if (i != currentPlayer){
			...
			//modyfing the bug I introduced which caused a dereferencing error thus producing no results
			//here we will simply make sure we never try to dereference -1. Original bug should still be
			//present for all values of i > 0
			//if(i == 0) state->deck[i][state->deckCount[i]--] = curse;//Correct implementation
			state->deck[i][state->deckCount[i]--] = curse;//Correct implementation
			//else state->deck[i][state->deckCount[i--]] = curse;//Bug -- //Top card now a curse
		}
	}
	return 0;
}
...
int TreasureMap(struct gameState *state, int handPos) {
	 int i;
	 int index;
	 int currentPlayer = whoseTurn(state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case treasure_map:
		return TreasureMap(state, handPos);
    }
